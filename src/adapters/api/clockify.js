...
export const fetchAllClients = async () => {
  try {
    const response = await clockifyAPI.get(
      `workspaces/${process.env.REACT_APP_WORKSPACE_CLOCKIFY}/clients`
    );
...
export const createClient = async (name) => {
  try {
    const response = await clockifyAPI.post(
      `workspaces/${process.env.REACT_APP_WORKSPACE_CLOCKIFY}/clients`,
      { name }
    );
    return response.data;
...
export const startTimeEntry = async (
  ...
  try {
    const response = await clockifyAPI.post(
      `workspaces/${process.env.REACT_APP_WORKSPACE_CLOCKIFY}/time-entries`,
      {
        start: startTime,
        projectId: projectId,
        clientId: clientId,
